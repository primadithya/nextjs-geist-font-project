...
try {
    ...
    header("Location: index.php?msg=Error fetching student data");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($no_pendaftaran)) {
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM students WHERE no_pendaftaran = ? AND id != ?");
        $stmt->execute([$no_pendaftaran, $id]);
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("UPDATE students SET nama_siswa = ?, asal_sekolah = ?, no_pendaftaran = ?, status_kelulusan = ? WHERE id = ?");
            $stmt->execute([$nama_siswa, $asal_sekolah, $no_pendaftaran, $status_kelulusan, $id]);
                ...
                <form method="POST">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="nama_siswa" name="nama_siswa" 
                               value="<?php echo htmlspecialchars($_POST['nama_siswa'] ?? $student['nama_siswa']); ?>" required>
                    </div>

                    <div class="mb-3">
                        <label for="asal_sekolah" class="form-label">School Name</label>
                        <input type="text" class="form-control" id="asal_sekolah" name="asal_sekolah" 
                               value="<?php echo htmlspecialchars($_POST['asal_sekolah'] ?? $student['asal_sekolah']); ?>" required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="no_pendaftaran" name="no_pendaftaran" 
                               value="<?php echo htmlspecialchars($_POST['no_pendaftaran'] ?? $student['no_pendaftaran']); ?>" required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="status_kelulusan" name="status_kelulusan" required>
                            ...
                            <option value="LULUS" <?php echo (($_POST['status_kelulusan'] ?? $student['status_kelulusan']) === 'LULUS') ? 'selected' : ''; ?>>LULUS</option>
                            <option value="TIDAK LULUS" <?php echo (($_POST['status_kelulusan'] ?? $student['status_kelulusan']) === 'TIDAK LULUS') ? 'selected' : ''; ?>>TIDAK LULUS</option>
                        </select>
                    </div>
